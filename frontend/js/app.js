...
async function submitAuth() {
  ...
  try {
    if (authMode === 'login') {
      ...
      if (res.statut === 'en_attente') { showPendingScreen(); return; }
      ...
      await Api.register(nom, tel, metier, pass);
      showPendingScreen();
    }
  } catch (err) {
...
async function startBotGreeting(artisan) {
  Bot.reset();
  setTimeout(()=>{ appendMessage('bot',`Bonjour ${artisan.nom} ! 👋 Je suis prêt à t'aider à créer un devis professionnel. Appuie sur "Nouveau devis" ou dis-moi directement : quel client aujourd'hui ?`); setQuickReplies(['Nouveau devis']); }, 600);
}

function newDevis() {
  const stored=localStorage.getItem('dp_artisan');
  if(stored){const a=JSON.parse(stored);if(a.plan==='gratuit'&&a.devis_count>=3){appendMessage('bot','🔒 Vous avez utilisé vos 3 devis gratuits. Abonnez-vous pour continuer.');setQuickReplies(['Contacter via WhatsApp']);return;}}
  Bot.reset(); document.getElementById('chat').innerHTML=''; setQuickReplies([]); hideVoiceConfirm(); showTyping();
  setTimeout(()=>{ hideTyping(); appendMessage('bot',"C'est parti ! Quel est le nom de ton client ?"); document.getElementById('msg-input').focus(); }, 500);
}
  ...
  try {
    ...
    if(finalAction){
      try {
        ...
        const stored=localStorage.getItem('dp_artisan');
        if(stored){const a=JSON.parse(stored);a.devis_count=(a.devis_count||0)+1;localStorage.setItem('dp_artisan',JSON.stringify(a));if(a.plan==='gratuit')showQuotaBanner(Math.max(0,3-a.devis_count));}
